...
write.table(blogs, "blog.txt", sep="\t")
...
### Tokenization Func ###
#########################
source('func/Task_1.5_Tokenization_func.R')
...
swearwords <- read.table('profanity filter/en', sep='\n')
names(swearwords)<-'swearwords'
filter <- rep('***', length(swearwords))
profanity <- data.frame(swearwords, target = filter)
rbind(profanity, data.frame(swearwords = c("[^[:alpha:][:space:]']","a ","a","e"), target = c(" ","'","'","'")))
